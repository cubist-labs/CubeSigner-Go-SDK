...
```bash
 go test -race -skip ^TestMfaSignEmail ./...
```

`TestMfaSignEmail` is an interactive test and must be compiled and
run separately:

```bash
go test -c ./... -v # compiles all tests.
./test.test -test.run TestMfaSignEmail
```

Email Otp is needed in this test and must be provided by the user in the
CLI.
